...
The goal of this lab is to demonstrate the cost reduction strategy implemented in the lab. In Falcon Next-Gen SIEM, ingestion costs are typically based on the volume of data ingested per day, meaning that sending every raw security log directly to the SIEM can quickly become expensive. In this architecture, Cribl Stream acts as a telemetry optimization layer between Suricata and Falcon NG-SIEM. Instead of forwarding every alert generated by the IDS, Cribl applies filtering, suppression, and sampling techniques to reduce duplicate or low-value events before they are ingested by the SIEM.
