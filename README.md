...
<img width="1846" height="923" alt="image" src="https://github.com/user-attachments/assets/708357cd-3d69-4135-a89e-7fb67d961c26" />
...
<img width="1211" height="923" alt="image" src="https://github.com/user-attachments/assets/3d8e8136-4141-4658-a568-45a5cbd0db67" />
...
<img width="1653" height="794" alt="image" src="https://github.com/user-attachments/assets/11d0f4d8-c121-4cb3-8499-c56b1657d49b" />
...
<img width="1747" height="734" alt="image" src="https://github.com/user-attachments/assets/843f7bdc-d193-4009-88c2-382f5de8a88e" />
...
<img width="1176" height="860" alt="image" src="https://github.com/user-attachments/assets/bc740037-032d-4de3-b9de-6ac5be8f3ce3" />
...
<img width="1671" height="664" alt="image" src="https://github.com/user-attachments/assets/ddbc7354-336c-41ab-8f7d-addfa5bebf54" />
...
<img width="1698" height="674" alt="image" src="https://github.com/user-attachments/assets/8deae753-413a-4ab3-8853-8ad4d37ced47" />
...
<img width="1747" height="730" alt="image" src="https://github.com/user-attachments/assets/f921173d-1c7a-4ab9-a9a5-f6d04a0349da" />
...
The query facilityName="local5" appname="suricata" | table([@timestamp, @rawstring]) is used in Falcon Next-Gen SIEM to search and display Suricata IDS alerts that were ingested through the logging pipeline. The first part of the query, facilityName="local5" appname="suricata", filters the dataset to return only events generated by Suricata that were sent through the syslog facility local5, which is the facility configured on the Suricata host when forwarding alerts via syslog to Cribl. This ensures that the results only include IDS-related logs from the detection system rather than other system logs. The pipe symbol | passes the filtered results to the next operation, which is the table() function. The table([@timestamp, @rawstring]) command formats the output into a simple table showing two key fields: @timestamp, which indicates the exact time the event was recorded by the SIEM, and @rawstring, which contains the original raw Suricata alert message, including the signature ID, classification, priority, and network connection details. This query therefore provides a concise view of Suricata detections over time, allowing analysts to quickly review when alerts occurred and inspect the full original alert message for investigation.

In the bar chart dashboard, we can observe that the number of events decreased drastically from 830 events at 18:00 to approximately 2–3 events afterward, as duplicate events were removed using the filtering functions.

<img width="1860" height="730" alt="image" src="https://github.com/user-attachments/assets/94ebee2e-2302-4eee-b52a-4a778c970f95" />
